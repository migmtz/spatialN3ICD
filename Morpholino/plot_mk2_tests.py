...
if __name__ == "__main__":
    ...
    mark_id_list = ['NOTCH mean', 'NOTCH sum']
    ...
    fig, ax = plt.subplots(2, 1, figsize=(5.5, 9), sharex=True, sharey=False)

    for id_mark, mark_id in enumerate(mark_id_list):
        print(mark_id)
        # Variables to read estimations
        ...
        for control_id in [0, 1]:
            ...
            for j, m_name in enumerate(func_name_list):
                ...
                ax[id_mark].plot(r_list, observed_Lfunc - estimated_randomL, linestyle='-', c=colors[control_id], label=name_list[control_id], # c="#440154",
                                 alpha=1.0)
                ax[id_mark].plot(r_list, D_max * np.ones(r_list.shape), linestyle='--', c=colors[control_id],
                                 alpha=1.0)
                ax[id_mark].plot(r_list, - D_max * np.ones(r_list.shape), linestyle='--', c=colors[control_id],
                                 alpha=1.0)
    ...
    fig.suptitle("MO | N3ICD Sum | Lower Activity group")

    fig.savefig("images/mo_test_of_randomness_lower.pdf", format="pdf", bbox_inches="tight")
