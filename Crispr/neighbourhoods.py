...
if __name__ == "__main__":
    ...
    for i in range(3):
        ...
        #sc = ax[control_id, i].scatter(voisinages[i][:, 0], voisinages[i][:, 1], c=voisinages[i][:, 2], s=10, cmap="YlGn", alpha=0.75)
        ax[i].set_title(str(np.round(np.mean(voisinages[i][:, 2] < 0.0), 2)) + ",  " + str(np.round(np.mean(voisinages[i][:, 2] > 0.0), 2)) + ", n=" + str(len(c)))
        #fig.colorbar(sc)
    #plt.colorbar(sc)
    print("estimated proportions: ", aux_low/n, aux_mid/n, aux_high/n)

    fig2.savefig("images/crispr_neighbourhoods.pdf", format="pdf")#, bbox_inches="tight")
    plt.show()